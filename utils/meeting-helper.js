...
async function joinMeeting(meetingId, socket, meetingServer, payload) {
    ...
    meetingSrvices.isMeetingPresent(meetingId, async (error, results) => {
        if (error && !results) {
            sendMessage(socket, {
...
function forwardConnectionRequest(meetingId, socket,meetingServer,payload){
    ...
    meetingSrvices.getMeetingUser(model,(error,results)=>{
        if(results){
...
function endMeeting(meetingId, socket,meetingServer,payload){
    ...
    meetingSrvices.getAllMeetingUsers(meetingId,(error,results)=>{
        for (let i = 0; i < results.length; i++) {
            ...
            meetingServer.sockets.connected[meetingUser.socketId].desconnect();
...
function addUser(socket, { meetingId, userId, name }) {
    let promise = new promise(function (resolve, reject) {
        meetingSrvices.getMeetingUser({ meetingId, userId }, (error, results) => {
            if (!results) {
                var model = {
                    socketId: socket.id,
                    meetingId: meetingId,
                ...
                meetingSrvices.joinMeeting(model, (error, results) => {
                    if (results) {
                        resolve(true);
                    }
                    if (error) {
                ...
                meetingSrvices.updateMeetingUser({
                    userId: userId,
                    socketId: socket.id,
                }, (error, results) => {
                    if (results) {
